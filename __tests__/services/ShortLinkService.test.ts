...
const count = jest.fn().mockReturnValue(0) as Models['ShortLinkView']['count'];
...
const shortLinkMetasByShortLinkIdLoader = new ShortLinkMetasByShortLinkIdLoader(
  db,
);
...
describe('ShortLinkService', () => {
  ...
  describe('#addShortLink', () => {
    ...
    it('should throw FullLinkNotOkError', async () => {
      await expect(
        shortLinkService.addShortLink({ fullLink: 'Invalid URL' }, {
          id: userId,
        } as User),
      ).rejects.toMatchInlineSnapshot(`
        [GraphQLError: Error: TypeError [ERR_INVALID_URL]: Invalid URL
        from: Invalid URL]
      `);
      ...
      await expect(
        shortLinkService.addShortLink({ fullLink: googleLink }, {
          id: userId,
        } as User),
      ).rejects.toMatchInlineSnapshot(`
        [GraphQLError: Error: Error: Mocked error
        from: https://www.google.com]
      `);

      expect(mockFetch).toHaveBeenLastCalledWith(new URL(googleLink));
    ...
    it('should add shortLink', async () => {
      ...
      expect((bulkCreate as jest.Mock).mock.calls[0][0]).toMatchInlineSnapshot(`
        [
          {
            "rawText": "
              &lt;head&gt;: The Document Metadata (Header) element - HTML: HyperText
