...
const ShortLinkModel = { findAll, create, findOne } as Models['ShortLink'];
...
describe('ShortLinkService', () => {
  ...
  describe('#getShortLinkBySlug', () => {
    it('should get shortLink by slug', async () => {
      expect(await shortLinkService.getShortLinkBySlug(slug, false)).toBe(null);
  ...
  describe('#loadShortLinksByUserId', () => {
    it('should load shortLinks by userId', async () => {
      ...
      expect(findAll).toHaveBeenLastCalledWith({ where: { userId: [userId] } });
    });
  });
